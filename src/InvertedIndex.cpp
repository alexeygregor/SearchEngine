...
void InvertedIndex::getDocuments( ConverterJSON& cvr )
{
    bool check = true;
    string request, word;
    ...
    if ( doc_id > 0 && doc_id <= cvr.getTextPath().size() )
    {
        ifstream Check( cvr.getTextPath()[doc_id - 1].c_str(), ios::in );
        Check >> word;
        if ( word.empty() )
        {
        ...
        ifstream Document( cvr.getTextPath()[doc_id - 1].c_str(), ios::binary );
